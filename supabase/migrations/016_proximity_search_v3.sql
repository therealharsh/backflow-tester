-- Migration: Update providers_near_point to include premium/claim fields
-- Added: is_premium, premium_plan, premium_rank, claimed, claim_status, claim_email
-- Must drop first because the return type changed (new columns).

DROP FUNCTION IF EXISTS providers_near_point(double precision, double precision, double precision, integer, text);

CREATE FUNCTION providers_near_point(
...
AS $$
  SELECT
    ...
    p.is_premium, p.premium_plan, p.premium_rank,
    p.claimed, p.claim_status, p.claim_email,
    3959 * acos(
      least(1.0,
...
-- Re-grant with the same 5-arg signature
